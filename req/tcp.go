...
func processTcp(conn *utils.FdConn) {
	...
	if err = mFdConn.Write(sendMsg, []byte{}, []byte(utils.EventAuth)); err != nil {
		conn.Log(fmt.Sprintf("目标服务：%s,远程服务认证失败: %v", address, err.Error()), log2.DebugError)
		return
	}
	authMsg := utils.MsgDecode(mFdConn.Conn)
	if authMsg.Err != nil {
		conn.Log(fmt.Sprintf("目标服务：%s,远程服务认证失败: %v", address, authMsg.Err.Error()), log2.DebugError)
		return
	}
	if string(authMsg.Msg) != "ok" {
		conn.Log(fmt.Sprintf("目标服务：%s,远程服务认证失败: %v", address, string(authMsg.Msg)), log2.DebugError)
		return
	}
	//连接建立成功
